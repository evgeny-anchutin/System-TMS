	...
	// -- 08.09.2015 Доработка от tristarr1
		...
		Для каждого СтрокаПакета Из ТаблицаПакетов Цикл
			...
			Если СтрокаПакета.ЭтоВТ Тогда
				// ++ Georg Georg (Georg_QQ) Исправление ошибки, возникающей при совпадении имен поля и ВТ.
			...
			Если пПоказать Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					Для СчКол = 1 По КоличествоКолонок Цикл
						...
						Если Оформление.Текст = "<РезультатЗапроса>" Тогда
							ОблЯчейки.СодержитЗначение = Ложь;
						КонецЕсли; 
						Если НЕ ОблЯчейки.СодержитЗначение Тогда
							ОблЯчейки.Текст = Оформление.Текст;
						КонецЕсли; 
						ОблЯчейки.ЦветТекста = Оформление.ЦветТекста;
						ОблЯчейки.Расшифровка = Оформление.Значение;
						//ОблЯчейки.ГоризонтальноеПоложение = Оформление.ГоризонтальноеПоложение;
				...
				Для СчКол = 1 По КоличествоКолонок Цикл
					...
					Для каждого ТипЗнач Из ТипыЗначений Цикл
						...
						ИначеЕсли Перечисления.ТипВсеСсылки().СодержитТип(ТипЗнач)Тогда
							...
							//ИначеЕсли  ТипЗнач = ТипДата Тогда
...
Процедура ПолучитьПараметрыЗапроса(Знач ТекстДляРазбора, ИД, СтруктураДанных = Неопределено) Экспорт
	...
	Если НЕ Пустаястрока(ТекстЗапроса) Тогда
		...
		Для каждого ПараметрЗапроса Из ПараметрыЗапроса Цикл
			...
			//////////		СтрокаПараметра.ОграничениеТипов.ОписаниеТипа = Новый ОписаниеТип3МоментВремени");
					СтрокаПараметра.Используется = Истина;
					Продолжить;
			...
			//////////		//
			//////////		ОписаниеТипаТЗ = ОТТаблицаЗначений;
			...
			//////////ИначеЕсли	ТипЗнч(СтрокаПараметра.Значение) = ТипДата
...
Функция ПараметрыДляОтображенияЯчейки(Значение, Хранилище, ОписаниеТипаЗначения) Экспорт
	...
	//Ячейка = Новый Структура("Значение,Текст,ЦветТекста,Ширина,ГоризонтальноеПоложение", Значение, Значение, WebЦвета.Черный, 0, ГоризонтальноеПоложение.Авто);
	Ячейка = Новый Структура("Значение,Текст,ЦветТекста,Ширина,Формат", Значение, Значение, WebЦвета.Черный, 0, "");
	
	Если ТипЗнч(Ячейка.Значение) = ТипБулево Тогда	
	...
	ИначеЕсли Ячейка.Значение = "" Тогда	
		...
		Ячейка.ЦветТекста = WebЦвета.Серый;
		Ячейка.Ширина = СтрДлина(Ячейка.Текст);
	ИначеЕсли ТипЗнч(Ячейка.Значение) = ТипМоментВремени Тогда	
		Ячейка.Текст = "<МоментВремени>";
		Ячейка.ЦветТекста = WebЦвета.Серый;
		Ячейка.Ширина = СтрДлина(Ячейка.Текст);
	ИначеЕсли ТипЗнч(Ячейка.Значение) = ТипРезультатЗапроса Тогда	
	...
	ИначеЕсли  Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(Ячейка.Значение)) 
		...
		ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(Ячейка.Значение))  Тогда	
			...
			//Ячейка.Ширина = Цел(СтрДлина(Ячейка.Текст) * 0.8);
			// определим по типу
		...
		КонецЕсли;
	ИначеЕсли ТипЗнч(Ячейка.Значение) = ТипСтрока Тогда	
	...
	ИначеЕсли Ячейка.Значение = Дата(1,1,1) Тогда
		...
		Ячейка.ЦветТекста = WebЦвета.Серый;
		Ячейка.Ширина = СтрДлина(Ячейка.Текст);
	ИначеЕсли ТипЗнч(Ячейка.Значение) = ТипДата Тогда	
		Ячейка.ЦветТекста = WebЦвета.ТемноСиний;
		Ячейка.Ширина = СтрДлина("00.00.0000 00:00:00");
	ИначеЕсли ТипЗнч(Ячейка.Значение) = ТипЧисло Тогда	
		Ячейка.ЦветТекста = WebЦвета.ФуксинТемный;
		РазрядностьДробнойЧасти = ОписаниеТипаЗначения.КвалификаторыЧисла.РазрядностьДробнойЧасти;
		Ячейка.Ширина = СтрДлина(Цел(Ячейка.Текст)) + ?(РазрядностьДробнойЧасти > 0, 1 + РазрядностьДробнойЧасти, 0);
	КонецЕсли;
